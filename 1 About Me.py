...
with experience:
    ...
    st.write("On the ShelfIQ team, I worked on adding additional functionality to the product by creating new \"actions\" (C#/.NET/Telerik) which are scripts that users can use to automate their planogram creation process. I also helped out in bugfixing for existing actions.")
    st.write("On the operations team, I worked directly with customer tickets through a CRM. I handled customer issues ranging from failing reports in MicroStrategy to data issues in our database and ETL process (SQL/Python/C#/.NET). On top of fixing their issues, I also facilitated communications back to the customer. In my time on the team, I handled over 50 customer tickets.")
